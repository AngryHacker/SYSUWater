...
(
	...
	`is_name` tinyint NOT NULL default 0 comment '是否匿名，0 = 否，1 = 是',
...
CREATE TABLE `Error`
(
	`er_id` INTEGER NOT NULL AUTO_INCREMENT comment '主键',
	`user_id` VARCHAR(124) NOT NULL comment '用户 ID',
	`psw` VARCHAR(124) NOT NULL comment '密码',
	primary key (`er_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 comment='记录错误表';

drop table if exists `score`;
CREATE TABLE `score`
(
	`sc_id` INTEGER NOT NULL AUTO_INCREMENT comment '主键',
	`num` VARCHAR(124) NOT NULL comment '积分分数',
	`time` date NOT NULL comment '积分日期',
	primary key (`er_id`)
